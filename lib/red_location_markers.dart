...
List<Marker> redLocationMarkers = (jsonDecode(File(cpDataPath).readAsStringSync())["red_locations"] as List<dynamic>).map(
  (e) {
    var redLocation = RedLocation.fromJson(e);

    if (redLocation.type == RedLocationType.policestation) {
      return Marker(
        point: LatLng(latFromNormY(redLocation.xy.y), lngFromNormX(redLocation.xy.x)),
    ...
    return Marker(point: LatLng(latFromNormY(redLocation.xy.y), lngFromNormX(redLocation.xy.x)), builder: (cx) => const Icon(Icons.dangerous));
  },
).toList();
