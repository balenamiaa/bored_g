...
void main() {
  ...
  var hospitals = hospitalRegex.allMatches(mapGroupPosStr).map((e) {
    ...
    return PinkLocation.fromXY(PinkLocationType.hospital, convertGameXToNormX(x), convertGameYToNormY(y));
  });
  ...
  var medicalcenters = medicalCenterRegex.allMatches(mapGroupPosStr).map((e) {
    ...
    return PinkLocation.fromXY(PinkLocationType.medicalcenter, convertGameXToNormX(x), convertGameYToNormY(y));
  });
  ...
  stdout.write(
    jsonEncode(
      ...
      toEncodable: (nonEncodable) => PinkLocation.toJson(nonEncodable as PinkLocation),
    ),
  );
}
